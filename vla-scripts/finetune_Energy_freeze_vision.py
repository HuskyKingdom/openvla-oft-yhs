...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        # compute energy loss ————————
        # Only extract vision patches (which includes proprio embedding if use_proprio=True)
        # num_patches already accounts for proprio if enabled (see line 1049-1051)
        vision_hidden = output.hidden_states[-1][:, :num_patches, :].detach()  # (B, num_patches, D)
    
        # Build mask for vision patches only (all False = all visible, no padding needed)
        vision_mask = torch.zeros(vision_hidden.shape[0], num_patches, dtype=torch.bool, device=vision_hidden.device)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            # Only pass vision patches and actions to EnergyModel
            swap_loss, E_pos_mean, E_neg_mean = energy_inbatch_swap_infonce(energy_model, vision_hidden, ground_truth_actions, vision_mask)
        ...
        if use_l1_regression:
            with torch.no_grad():
                ...
                # 返回一个 0 标量以兼容上层逻辑
                loss = torch.zeros((), device=device_id, requires_grad=True)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if (cfg.use_l1_regression or cfg.use_diffusion):
        freeze_module(action_head)
    if cfg.use_proprio:
        freeze_module(proprio_projector)
    if cfg.use_diffusion:
        freeze_module(noisy_action_projector)
